...
def apply_classifier(x, model, img, im0,names):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            times.append(t2-t1)
            # get only where class is person
            indices_person=(pred_cls1==0)
            pred_cls2=pred_cls2[indices_person]
            pred_cls2=len(names)-(pred_cls2)-1
            # If the class is person change it to appropriate subclass
            #print(len(names))
            #print(x[i].shape)
            #print(x[i])
            x[i][indices_person,5]=pred_cls2.float()
            #print(x[i])
...
def draw_blend_bbox(x, img):
    ...
    for bbox in x:
        ...
        conf=bbox[4]
        clss= bbox[5]
        cv2.rectangle(blank_image, c1, c2, c, -1)
    alpha=0.5
    beta = (1.0 - alpha)
    img= cv2.addWeighted(img, alpha, blank_image, beta, 20.0)
    return img
